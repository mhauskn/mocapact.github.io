...
Specify a type `-t` from `<experts | small_dataset | larget_dataset>`, a comma-separated list of clips `-c` (e.g., `CMU_001_01,CMU_002_01`) or a specific subset from `dm_control`'s [MoCap subsets](https://github.com/deepmind/dm_control/blob/main/dm_control/locomotion/tasks/reference_pose/cmu_subsets.py) of form `<get_up | walk_tiny | run_jump_tiny | locomotion_small | all>`, and a destination path `-d`.
For example:
```bash
...
## Multi-Clip Policy
We then train a multi-clip policy on the dataset to track all the clips in `dm_control`.
