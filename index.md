...
In the videos, the prompt is shown by the red humanoid.
From there, the GPT policy controls the humanoid (visualized in bronze).

To help compare the behavior between GPT and the MoCap clip, we collect the actions executed in a rollout from both the GPT policy and an expert policy.
From the collected actions, we form action sequences of length 32 for both the GPT policy and the expert.
We then perform PCA on these sequences and provide 3-dimensional visualizations of the action sequences.
